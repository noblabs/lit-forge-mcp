// 経済指標リリース予定 リアルタイム取得ツール（v0.11.0 新設）。
//
// 役割分担:
//   - 本ツール: 公式機関のリリースカレンダーを実取得し、収録漏れの無い「網羅性」を担保
...
// 次サイクル予定: NY 連銀（Empire State）/ FRB（鉱工業生産・FOMC）/ BLS / 総務省統計局。
...
export const getEconomicReleasePulseTool: LitForgeTool = {
  ...
  description:
    ...
    "BEA（米経済分析局）・内閣府・日銀の公式日程を並列取得し、国・期間でフィルタして返します。" +
    "【get_economic_events_today との違い】" +
    "`get_economic_events_today` は運営者が一次ソースで裏取りした手動キュレーション（★★★ 中心、半年 1 回更新）で、" +
    "収録対象を絞っているため NY 連銀製造業や鉱工業生産などを取りこぼす『網羅性の穴』があります。" +
    "本ツールは公式カレンダーを実取得することでその穴を埋める**補完ツール**です。市況サマリーでは両方を併用してください。" +
    "【現行カバー範囲（v0.11.0）】" +
    "BEA（米 GDP / 個人所得・支出〔PCE〕/ 貿易収支 / 企業収益）、内閣府（四半期別 GDP 速報）、" +
    "日銀（金融政策決定会合 / 主な意見 / 短観 / 議事要旨）。" +
    "NY 連銀（Empire State）・FRB（鉱工業生産・FOMC）・BLS（雇用統計・CPI・PPI）・総務省統計局（日 CPI）は次サイクルで追加予定で、現時点では本ツールに含まれません。" +
    "【利用上の注意】" +
